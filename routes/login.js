...
module.exports.Router = class Routes extends Router {
	constructor() {
		...
		this.get('/', async function (req, res) {
			const URL = `${req.config.publicurl}/login`;

			if (!req.query.code) return res.redirect(`https://discord.com/api/oauth2/authorize?client_id=${req.client.user.id}&scope=identify%20guilds&response_type=code&redirect_uri=${encodeURIComponent(URL)}&prompt=none`);
